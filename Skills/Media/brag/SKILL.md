...
**Thoth OS Dependency Check (REQUIRED):**
Before proceeding with project inspection, you must verify the required dependencies for rendering:
1. Detect `Node.js` (`node -v`)
2. Detect `FFmpeg` (`ffmpeg -version`)
3. Detect `Hyperframes CLI` (`npx hyperframes -v` or checking if the package runs)

If any of these dependencies are unavailable, you MUST immediately return a clear actionable diagnostic and abort the skill execution. For example:
`/brag is installed correctly. Rendering cannot continue because Hyperframes is unavailable. Detected: ✓ Node.js ✓ FFmpeg ✗ Hyperframes Run: npx hyperframes doctor`
Do NOT mock the rendering pipeline or pretend a video was generated if rendering failed.
...
**Windows Support (Thoth OS Requirement):**
When executing file operations (e.g., `mkdir`, `cp`, `mv`) or running chained commands (e.g., `&&`, `\`), ensure you use the correct syntax for the user's operating system (PowerShell on Windows). For example, use `New-Item -ItemType Directory -Force` instead of `mkdir -p`, `Copy-Item` instead of `cp`, `` ` `` for line continuations instead of `\`, and `;` or explicit `if($?)` instead of `&&`.
...
## Step 1: Inspect the project (Thoth Context Bridge)

**Read:** [references/step-1-inspect.md](references/step-1-inspect.md)

As a Thoth OS agent, you have access to the project's broader context. Scan the project directory and extract the information needed to plan the brag video. Leverage the Thoth `Context Manager` to:
- Read `README.md`, `package.json`, and other metadata files.
- Understand the core project identity (product name, tagline, brand colors, logo).
- Identify key visual moments, UI structure, and major features.
- If available, utilize screenshots or live/demo URLs.

The goal is to understand WHAT WAS BUILT, WHY IT EXISTS, WHAT MAKES IT INTERESTING, and WHAT THE STORY SHOULD BE.

**Gate:** You can answer all 9 questions in the brag planning rubric using project-specific context.
